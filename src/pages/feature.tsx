    ...
    <div className="h-screen bg-gray-100 container mx-auto">
      <div className="flex justify-between items-center pt-20">
        <div className="text-center mx-auto pl-80">
          ...
          <p className="text-[#9A3412] bg-[#FFEDD5] rounded-2xl inline px-3 py-1">
            放送前・エンジビア募集中
          </p>
          <h1 className="text-[#111827] font-bold text-3xl mt-4">
            第４回エンジビアの泉
          </h1>
        </div>
        <div>
          <button className="bg-[#0284C7] text-white px-8 py-3 rounded-md">
            放送を開始する
          </button>
          <button className="px-8 py-3 rounded-md bg-[#E0F2FE] text-[#0369A1] ml-8">
            編集する
          </button>
        </div>
      </div>
      <div className="mx-2 mt-16">
        <div className="flex justify-between">
          {FeatureText?.map((text) => (
            <p
              key={text}
              className="text-center w-96 h-14 pt-4 bg-gray-300 text-[#111827] rounded-lg"
            >
              {text}
            </p>
          ))}
        ...
        <div className="flex justify-between">
          <div>
            {beforeFeatureData.map((data) => (
              <div
                className="bg-white w-96 h-24 rounded-lg p-4 mt-4"
                key={data.id}
              >
                ...
                <div className="flex items-center pt-4">
                  <img
                    ...
                    alt="avatar"
                  />
                  <p className="text-xs text-[#374151] pl-4">{data.name}</p>
                </div>
              </div>
            ))}
          </div>
          <div>
            ...
            <button className="bg-[#0284C7] text-white px-6 py-3 rounded-md block mx-auto mt-4">
              タイトルコールする
            </button>
          </div>
          <div>
